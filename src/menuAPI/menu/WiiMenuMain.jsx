...
export function WiiMenuMain({ mode }) {
		...
		function handleKeyPress(e) {
			if (e.key === "Enter") {
				...
				setTimeout(() => {
					...
					localStorage.setItem("healthAndSafetyOpen", false);
				}, 1000);
			}
		}
	...
	if (!started) {
		localStorage.setItem("healthAndSafetyOpen", true);
		return (
			<div
			...
			>
				...
				<p className="text-xs">More info at</p>
				...
				<audio
					ref={audioRef}
					src="/assets/nintendo/audio/wiimenu/NoA_OpenChannel.wav"
					preload="auto"
				/>
			</div>
		);
	...
	if (
		localStorage.getItem("inChannel") !== "true" &&
		localStorage.getItem("inChannel") !== "false"
	) {
		localStorage.setItem("inChannel", "false");
	}

	if (
		localStorage.getItem("homeMenuOpen") === "false" &&
		localStorage.getItem("inChannel") === "false"
	) {
		wiiPlayAudio({
